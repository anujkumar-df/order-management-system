"""Integration tests for the CancelOrder use case."""
...
class TestCancelConfirmedOrder:
    ...
    def test_cancel_releases_inventory(self):
        ...
        cancel.handle(dto.id)

        # Order is cancelled
        order = order_repo.get_by_id(dto.id)
        assert order.status == OrderStatus.CANCELLED

        # Inventory is fully released
        assert inventory_repo.get_by_product_id("1").reserved_quantity == 0
        assert inventory_repo.get_by_product_id("1").available_quantity == 100
        assert inventory_repo.get_by_product_id("2").reserved_quantity == 0
        assert inventory_repo.get_by_product_id("2").available_quantity == 50


class TestCancelDraftOrder:

    def test_cancel_draft_does_not_touch_inventory(self):
        order_repo, product_repo, inventory_repo = _setup()

        create = CreateOrderHandler(order_repo, product_repo)
        dto = create.handle("Alice", [OrderItemSpec("Widget", 10)])
...
class TestCancelValidation:
    ...
    def test_cancel_nonexistent_order_rejected(self):
        order_repo, _, inventory_repo = _setup()
        handler = CancelOrderHandler(order_repo, inventory_repo)
